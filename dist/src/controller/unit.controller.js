...
export default function UnitController() {
    ...
    async function createUnit(req, res) {
        try {
            const { unitNo, category, brand, description, ownerName, km, hourMeter, status, } = req.body;
            if (!unitNo || !category || !brand || !description || !ownerName || km === undefined) {
                return res.status(400).json({
                    success: false,
                    message: 'Data unit tidak lengkap: unitNo, category, brand, description, ownerName, dan km wajib diisi',
                });
            }
