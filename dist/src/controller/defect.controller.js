 ...
 */
export function extractDefectsFromInspection(inspection) {
    const defects = [];
    ...
    if (Array.isArray(inspection.damageChecks)) {
        inspection.damageChecks.forEach((chk, idx) => {
            const cond = String(chk.condition || '').toUpperCase();
            const isDefect = cond === 'MAJOR' ||
                cond === 'RUSAK' ||
                cond === 'TIDAK BAIK' ||
                cond === 'TIDAK_BAIK' ||
                cond === 'TIDAK NORMAL' ||
    ...
    if (inspection.tyreCheck && typeof inspection.tyreCheck === 'object') {
        ...
        const isTyreDefect = tc.condition === 'BOTAK' ||
            tc.condition === 'RUSAK' ||
            tc.pressure === 'KURANG' ||
    ...
    if (Array.isArray(inspection.safetyTools)) {
        const missingTools = inspection.safetyTools.filter((t) => t.status === 'TIDAK_ADA' || t.status === 'TIDAK');
        if (missingTools.length > 0) {
            const toolNames = missingTools.map((t) => t.item).join(', ');
            defects.push({
...
export async function getAllDefects(req, res) {
    try {
        const { category, status, severity, search, page = 1, limit = 50, } = req.query;
        ...
        if (search) {
            ...
            allDefects = allDefects.filter((d) => d.unitNo.toLowerCase().includes(s) ||
 ...
 */
export async function getDefectStats(_req, res) {
    try {
        const inspections = await prisma.p2HInspection.findMany({
            include: {
                unit: {
 ...
 */
export async function updateDefectStatus(req, res) {
    try {
        const { defectId } = req.params;
        const { status, // 'OPEN' | 'IN_PROGRESS' | 'RESOLVED' | 'CLOSED'
        mechanicName, repairNotes, } = req.body;
        ...
        let updatedDamageChecks = inspection.damageChecks;
        if (Array.isArray(updatedDamageChecks) && idStr.includes('-DC-')) {
            ...
            if (dcIndexMatch) {
                ...
                const currentItem = updatedDamageChecks[itemIdx];
        ...
        const currentWd = inspection.warningDetails || {};
        const updatedWd = {
            ...currentWd,
            status,
        ...
        await prisma.p2HInspection.update({
            where: { id: inspectionId },
            data: {
                damageChecks: updatedDamageChecks,
                warningDetails: updatedWd,
 ...
 */
export async function createDirectBreakdown(req, res) {
    try {
        const userId = req.user?.id || 1;
        const { unitId, component, details, severity = 'CRITICAL', driverName, driverNrp, shift = 'PAGI', km, hourMeter, } = req.body;
