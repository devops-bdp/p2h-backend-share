...
export const CATEGORY_P2H_TOKENS: { [category: string]: string } = {
 ...
 */
function generateP2HNo(category: string): string {
  const dateStr = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const randomSuffix = Math.floor(1000 + Math.random() * 9000);
  let prefix = 'UNIT';
  if (category === 'LIGHT_VECHICLE') prefix = 'LV';
  else if (category === 'TELEHENDLER') prefix = 'TH';
  else if (category === 'STORING_TRUCK') prefix = 'ST';
 ...
 */
export function checkP2HToken(
  req: Request,
  targetCategory?: string
): { valid: boolean; message?: string } {
  const token =
    req.headers['x-p2h-token'] ||
  ...
  if (targetCategory && CATEGORY_P2H_TOKENS[targetCategory]) {
    ...
    const matchingCat = Object.keys(CATEGORY_P2H_TOKENS).find(
      (cat) => CATEGORY_P2H_TOKENS[cat] === tokenStr
    );
    if (matchingCat) {
      return {
        valid: false,
 ...
 */
export async function getPublicP2HOptions(req: Request, res: Response) {
  try {
 ...
 */
export async function createPublicP2HInspection(req: Request, res: Response) {
  try {
    const {
      unitId,
      driverId,
 ...
 */
export async function getAllP2HInspections(req: AuthRequest, res: Response) {
  try {
    const {
      unitId,
      unitNo,
 ...
 */
export async function getP2HInspectionById(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
 ...
 */
export async function createP2HInspection(req: AuthRequest, res: Response) {
  try {
 ...
 */
export async function updateP2HInspection(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    const {
      driverName,
 ...
 */
export async function deleteP2HInspection(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    const existing = await prisma.p2HInspection.findUnique({
      where: { id: Number(id) },
 ...
 */
export async function getP2HStats(req: AuthRequest, res: Response) {
  try {
