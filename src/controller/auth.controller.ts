...
export default function AuthController() {
    ...
    try {
      ...
      if (req.user && req.user.role === 'ADMIN' && role === 'SUPERADMIN') {
        return res.status(403).json({
          success: false,
          message:
            'Akses ditolak: User dengan role ADMIN tidak diizinkan mendaftarkan SUPERADMIN',
        });
      }
      ...
      return res.status(500).json({
        success: false,
        message:
          error.message || 'Terjadi kesalahan pada server saat registrasi',
      });
    }
  }
   ...
   */
  async function getDrivers(req: AuthRequest, res: Response) {
    try {
