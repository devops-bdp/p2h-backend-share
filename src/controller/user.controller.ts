...
export default function UserController() {
   ...
   * Reset Password User
   */
  async function resetUserPassword(req: AuthRequest, res: Response) {
    try {
      const id = Number(req.params.id);
  ...
  function normalizeRole(role?: string): 'SUPERADMIN' | 'ADMIN' | 'USER' {
    ...
    if (upper === 'SUPERADMIN' || upper === 'SUPER ADMIN' || upper === 'SUPER_ADMIN') return 'SUPERADMIN';
   ...
   */
  function normalizeDepartment(dept?: string): any {
    if (!dept) return 'HRGA';
    const upper = String(dept).trim().toUpperCase();
    if (['OPERATIONS', 'OPERASIONAL', 'PRODUKSI & TAMBANG', 'OPS'].includes(upper)) return 'OPERATIONS';
    if (['PRODUCTION_AND_ENGINEERING', 'PRODUCTION & ENGINEERING', 'ENGINEERING'].includes(upper)) return 'PRODUCTION_AND_ENGINEERING';
    if (['PLANT', 'MAINTENANCE', 'PLANT & MAINTENANCE'].includes(upper)) return 'PLANT';
    if (['LOGISTIC', 'LOGISTIK', 'SUPPLY CHAIN'].includes(upper)) return 'LOGISTIC';
    if (['HSE', 'K3', 'SAFETY', 'SHE'].includes(upper)) return 'HSE';
    if (['HRGA', 'HR', 'GA', 'HUMAN RESOURCE'].includes(upper)) return 'HRGA';
    return 'HRGA';
  }
   ...
   */
  function normalizePosition(pos?: string): any {
    if (!pos) return 'OPERATOR';
    const upper = String(pos).trim().toUpperCase().replace(/[\s-]/g, '_');
    if (['SITE_MANAGER', 'PROJECT_MANAGER', 'PM'].includes(upper)) return 'SITE_MANAGER';
    if (['SITE_SUPERVISOR', 'SUPERVISOR', 'PENGAWAS', 'SPV'].includes(upper)) return 'SITE_SUPERVISOR';
    if (['SITE_SUPERINTENDENT', 'SUPERINTENDENT', 'SUPT'].includes(upper)) return 'SITE_SUPERINTENDENT';
    if (['OPERATOR', 'OPERATOR_ALAT_BERAT'].includes(upper)) return 'OPERATOR';
    if (['MECHANIC', 'MEKANIK', 'FITTER'].includes(upper)) return 'MECHANIC';
    if (['ELECTRICIAN', 'ELEKTRIKAL', 'LISTRIK'].includes(upper)) return 'ELECTRICIAN';
    if (['TYREMAN', 'TYRE_MAN', 'BAN'].includes(upper)) return 'TYREMAN';
    if (['DRIVER', 'PENGEMUDI', 'SOPIR'].includes(upper)) return 'DRIVER';
    if (['ADMIN', 'STAFF_ADMIN', 'ADMINISTRATOR'].includes(upper)) return 'ADMIN';
    return 'OPERATOR';
  }
   ...
   */
  async function bulkCreateUsers(req: AuthRequest, res: Response) {
    try {
      const usersData: any[] = Array.isArray(req.body)
        ? req.body
        : req.body.users;
      ...
      for (let i = 0; i < usersData.length; i++) {
        ...
        const rawPassword =
          item.password && String(item.password).trim() !== ''
