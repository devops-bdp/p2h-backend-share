...
export async function authenticateToken(
  ...
  if (!token) {
    return res.status(401).json({
      success: false,
      message:
        'Akses ditolak: Token autentikasi tidak ditemukan. Harap sertakan token Bearer.',
    });
  }
...
export function authorizeDepartments(...allowedDepartments: string[]) {
  return async (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
      return res.status(401).json({
        success: false,
 ...
 */
export async function authorizeUnitManagement(
  req: AuthRequest,
  res: Response,
  next: NextFunction
) {
